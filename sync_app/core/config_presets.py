"""پریست‌های نام‌دارِ کلِ تنظیمات — امکانِ ذخیره‌ی چند مجموعه تنظیماتِ
...
هر پریست کاملاً مستقل و مکتفی‌به‌خودشه: یک snapshot از کلِ دیکشنریِ
...
def snapshot_current_config(config: dict) -> dict:
    """کپیِ کاملِ کانفیگِ فعلی، بدونِ کلیدهای داخلی/خودِ لیستِ پریست‌ها."""
...
def save_preset(config: dict, title: str, *, preset_id: str = "") -> dict:
    """پریستِ جدید می‌سازه یا (اگه preset_id یکی از پریست‌های موجود باشه)
    همون رو آپدیت می‌کنه. یه کپیِ جدید از config با لیستِ پریست‌های
...
def apply_preset(config: dict, preset: dict) -> dict:
    """اسنپ‌شاتِ پریست رو به‌عنوانِ کانفیگِ فعال برمی‌گردونه — لیستِ خودِ
    پریست‌ها (که جزوِ اسنپ‌شات نیست) از config فعلی حفظ می‌شه تا با
    اعمالِ یک پریست، بقیه‌ی پریست‌ها گم نشن."""
