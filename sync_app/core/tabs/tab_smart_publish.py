...
class SmartPublishSettingsTab(QWidget):
    ...
    def _build_auto_group(self) -> QGroupBox:
        ...
        note = QLabel(
            "ℹ️ این «روش پردازش تصویر» رویِ همه‌ی مسیرهایِ ارسالِ تصویر اعمال می‌شه — سینکِ کاملِ "
            "محصولات، ارسالِ دستی/گروهیِ تصویر از تبِ محصولات، و تصویرِ دسته‌بندی‌ها. برای اجرای "
            "دستیِ یک روشِ دیگر روی یک محصولِ خاص (بدونِ تغییرِ این تنظیمِ سراسری)، از دکمه‌ی 🎨 "
            "روی همان ردیفِ محصول در تبِ محصولات استفاده کنید."
        )
        note.setWordWrap(True)
        note.setStyleSheet("color:#64748b; font-size:11px;")
        layout.addWidget(note)
