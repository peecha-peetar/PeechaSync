# هشدار قبل از بازنویسی پروفایل سایت پرستاشاپ — هم‌ساختار با wc_site_edit_guard.py
...
def snapshot_ps_site(site: dict | None) -> dict:
    if not isinstance(site, dict):
        return {}
    out = {}
    for key in _COMPARE_FIELDS:
        val = site.get(key)
        if key in ("verify_ssl", "currency_is_toman"):
            out[key] = bool(val)
        elif key in ("lang_id", "root_category_id"):
...
def ps_site_form_changed(baseline: dict, form: dict) -> bool:
    base = snapshot_ps_site(baseline)
    cur = snapshot_ps_site(form)
    return base != cur


def ps_site_credentials_changed(baseline: dict, form: dict) -> bool:
    """تغییرات مهم اتصال — بدون نام پروفایل."""
    base = snapshot_ps_site(baseline)
    cur = snapshot_ps_site(form)
    for key in ("url", "api_key", "verify_ssl", "lang_id", "root_category_id", "currency_is_toman"):
        if base.get(key) != cur.get(key):
            return True
    return False


def ps_site_is_saved_profile(baseline: dict) -> bool:
    """پروفایلی که قبلاً URL یا کلید API داشته «ذخیره‌شده» محسوب می‌شود."""
    base = snapshot_ps_site(baseline)
    return bool(base.get("url") or base.get("api_key"))


def ps_site_hosts_differ(baseline: dict, form: dict) -> bool:
    old_host = ps_store_host(url=snapshot_ps_site(baseline).get("url", "")).lower()
    new_host = ps_store_host(url=snapshot_ps_site(form).get("url", "")).lower()
...
class PsSiteEditGuardDialog(QDialog):
    def __init__(
        self,
        parent=None,
        *,
...
def confirm_ps_site_edit(
    ...
    *,
    ...
    """
    baseline = snapshot_ps_site(baseline_site)
    if not ps_site_is_saved_profile(baseline):
        return ACTION_PROCEED
    if not ps_site_credentials_changed(baseline_site, form_site):
        return ACTION_PROCEED

    cross = ps_site_hosts_differ(baseline_site, form_site)
    old_host = ps_store_host(url=baseline.get("url", ""))
    new_host = ps_store_host(url=snapshot_ps_site(form_site).get("url", ""))
    label = str(baseline.get("label") or "").strip() or site_display_label(baseline_site)

    dlg = PsSiteEditGuardDialog(
        parent,
        site_label=label,
        old_host=old_host,
        new_host=new_host,
