...
class SettingsTab(QWidget):
    ...
    def _refresh_mobile_photo_info(self):
        ...
        # آدرسِ «افزودن به صفحه‌ی اصلیِ گوشی» شاملِ همین آی‌پی می‌شه و برایِ همیشه
        # همون آی‌پی رو نگه می‌داره — اگه بعداً آی‌پیِ این کامپیوتر (مثلاً با
        # اتصالِ دوباره به وای‌فای یا رفرشِ DHCP) عوض بشه، آیکونِ قبلیِ رویِ گوشی
        # دیگه به هیچ آدرسی وصل نمی‌شه (انگار برنامه اصلاً باز نمی‌شه). این‌جا
        # تغییرِ آی‌پی رو نسبت به آخرین باری که دیده شده تشخیص می‌دیم و به کاربر
        # هشدار می‌دیم که آیکونِ قدیمی رو حذف و دوباره با QR تازه اضافه کنه.
        last_ip = str(cfg.get("MOBILE_PHOTO_LAST_KNOWN_IP") or "").strip()
        ip_changed_warning = ""
        if ip and last_ip and last_ip != ip:
            ip_changed_warning = (
                f"\n\n🔴 آی‌پیِ این کامپیوتر عوض شده (قبلاً {last_ip} بود، الان {ip} است) — "
                "هر آیکونِ «افزودن به صفحه‌ی اصلی»یِ قبلی روی گوشی‌ها دیگه کار نمی‌کنه (چون به آدرسِ "
                "قدیمی وصل می‌شه، انگار برنامه باز نمی‌شه). آیکونِ قدیمی رو از گوشی حذف کنید و با "
                "QR کدِ تازه‌یِ زیر دوباره اضافه کنید. برایِ اینکه این اتفاق دیگه نیفته، از "
                "تنظیماتِ روتر یه آی‌پیِ ثابت (Static/Reserved IP) برایِ این کامپیوتر بگیرید."
            )
        if ip and ip != last_ip:
            cfg["MOBILE_PHOTO_LAST_KNOWN_IP"] = ip
    ...
    def init_ui(self):
        ...
        self.font_size_combo.addItem("خیلی بزرگ ضخیم (18px)", "18bold")
        ...
        self.wp_app_password_input.setPlaceholderText("رمزِ برنامه‌ی وردپرس (Application Password)")
        self.wp_app_password_input.set_toggle_tooltip_base("رمزِ برنامه")
