...
$killPs = Join-Path $PSScriptRoot 'kill-peecha-processes.ps1'
if (Test-Path -LiteralPath $killPs) {
    & $killPs -InstallDirs @($root) -WaitSeconds 3 | Out-Null
} else {
    $libKill = Join-Path (Split-Path -Parent (Split-Path -Parent $PSScriptRoot)) '_lib\kill-peecha-processes.ps1'
    if (Test-Path -LiteralPath $libKill) {
        & $libKill -InstallDirs @($root) -WaitSeconds 3 | Out-Null
    } else {
...
function Remove-TreeRetry([string]$path, [int]$attempts = 5) {
    ...
    for ($i = 0; $i -lt $attempts; $i++) {
        try {
            ...
            $killPs = Join-Path $PSScriptRoot 'kill-peecha-processes.ps1'
            if (Test-Path -LiteralPath $killPs) {
                & $killPs -InstallDirs @($root) -WaitSeconds 2 | Out-Null
            } else {
                Get-Process pythonw, python -ErrorAction SilentlyContinue | Where-Object {
                    $_.Path -and ($_.Path -like "*PeechaSync*")
                } | ForEach-Object {
